...
#         writer.writerow(["Timestamp", "Person ID", "Item", "Event", "Confidence"])

# # Construct argument parser
# ap = argparse.ArgumentParser()
# ap.add_argument("-p", "--prototxt", required=True, help="path to Caffe 'deploy' prototxt file")
# ap.add_argument("-m", "--model", required=True, help="path to Caffe pre-trained model")
# ap.add_argument("-c", "--confidence", type=float, default=0.3, help="minimum probability to filter weak predictions")
# ap.add_argument("-r", "--resolution", type=str, default="640x480", help="video resolution (e.g., 640x480)")
# ap.add_argument("-s", "--skip-frames", type=int, default=2, help="number of frames to skip between processing")
# args = vars(ap.parse_args())

# # Initialize class labels and colors
# CLASSES = ["aeroplane", "background", "bicycle", "bird", "boat", "bottle", "bus", "car", "cat", "chair", 
...
# fps = FPS().start()
# tracked_objects = {}
# taken_items = {}  # To track items taken and not yet returned
# person_id_counter = 0
# person_ids = {}  # To assign persistent person IDs
# frame_count = 0
...
# def assign_person_id(box):
#     """Assign a persistent person ID based on bounding box proximity."""
#     global person_id_counter
#     startX, startY, endX, endY = box
#     for pid, data in person_ids.items():
#         px, py, _, _ = data["box"]
#         if abs(px - startX) < 50 and abs(py - startY) < 50:
#             data["box"] = box  # Update position
#             return pid
#     person_id_counter += 1
#     person_ids[f"person_{person_id_counter}"] = {"box": box}
#     return f"person_{person_id_counter}"

# def log_event(person_id, item, event, confidence):
#     """Log event to console and CSV."""
#     timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
#     logger.info(f"{timestamp} - {person_id} {event} {item} (Confidence: {confidence:.2f})")
#     with open(CSV_FILE, mode='a', newline='') as file:
#         writer = csv.writer(file)
#         writer.writerow([timestamp, person_id, item, event, f"{confidence:.2f}"])
...
#             idx = classes[i]
#             confidence = scores[i]
#             obj_id = f"{CLASSES[idx]}_{startX}_{startY}"
...
#                 obj_id = assign_person_id((startX, startY, endX, endY))
            
#             current_detections[obj_id] = {"class": CLASSES[idx], "box": (startX, startY, endX, endY), "confidence": confidence}
...
#                 if obj_data["class"] == "bottle" and "person_id" in obj_data:
#                     taken_items[obj_id] = {
...
#                         "frame": frame_count
#                     }
#                     log_event(obj_data["person_id"], obj_data["class"], "taken", obj_data["confidence"])
#                     cv2.putText(frame, f"{obj_data['person_id']} took {obj_data['class']}", (10, 30), 
#                                 cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
#                 del tracked_objects[obj_id]

#         # Check for returned items
#         for obj_id, obj_data in current_detections.items():
#             if obj_data["class"] == "bottle" and obj_id in taken_items:
#                 person_id = taken_items[obj_id]["person_id"]
#                 log_event(person_id, obj_data["class"], "returned", obj_data["confidence"])
#                 cv2.putText(frame, f"{person_id} returned {obj_data['class']}", (10, 60), 
...
#                         if abs(p_startX - startX) < 50 and abs(p_startY - startY) < 50:
...
# #     """Assign a persistent ID based on centroid proximity."""
# #     startX, startY, endX, endY = box
...
# #         if data["class"] == cls:
# #             px = data["box"]  # px is the box tuple (startX, startY, endX, endY)
